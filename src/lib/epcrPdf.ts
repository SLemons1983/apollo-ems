...
function blocks(chart: Record<string, unknown>): Block[] {
  ...
  return [
    ...
    { title: 'Complaint', rows: rows(row('Chief Complaint', complaint.chiefComplaint), row('Clinical Impression', complaint.clinicalCategory), row('Clinical Differential', impression(complaint.primaryImpression)), row('Primary Symptom', impression(complaint.primarySymptom)), row('Associated Symptoms', complaint.otherAssociatedSymptoms), row('Symptoms Began', dateTime(complaint.symptomsBeganDateTime)), row('Last Known Well', dateTime(complaint.lastSeenNormalDateTime)), row('Patient Acuity', complaint.patientAcuity), row('Possible Trauma', complaint.possibleInjuryTrauma), row('Cardiac Arrest', complaint.cardiacArrest), row('Suspected Stroke', complaint.suspectedStrokeCva), row('Behavioral Hold', complaint.patientPlacedOn5150Hold), row('Drug/Alcohol', complaint.possibleDrugAlcoholUse)) },
