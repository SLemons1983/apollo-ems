...
export default function EpcrAccessManager({ agencyId, enabled }: { agencyId: string; enabled: boolean }) {
  ...
  async function invite(event: FormEvent) {
    event.preventDefault(); setMessage('Adding ePCR access…');
    ...
    setMessage(response.ok ? (result.reused_existing_account ? `${result.member.email} already has an Apollo ePCR login. Access to this agency was added without sending another invitation.` : `Invitation sent to ${result.member.email}. Username: ${result.member.username}`) : result.error);
