...
export default function ContinuingEducationPage(){
  ...
  async function deleteClass(ce:CeClass){
    const confirmed=window.confirm(`Delete "${ce.topic}"?\n\nThis will permanently delete this CE class and all associated attendance records. This cannot be undone.`);
    if(!confirmed)return;
    setStatus(`Deleting ${ce.topic}...`);
    try{
      const token=await accessToken();
      const response=await fetch(`/api/continuing-education/records?classId=${encodeURIComponent(ce.id)}`,{method:'DELETE',headers:{Authorization:`Bearer ${token}`}});
      const result=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(result.error||'Unable to delete CE class.');
      setClasses(current=>current.filter(item=>item.id!==ce.id));
      setAttendance(current=>{const next={...current};delete next[ce.id];return next;});
      setStatus(`Deleted CE class: ${ce.topic}.`);
    }catch(e:any){setStatus(`Unable to delete CE class: ${e.message}`);}
  }
