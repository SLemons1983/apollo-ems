...
export default function EmployeeProfilesPage() {
  ...
  function renderSummaryCard(
    ...
    const isOpen = activeSummaryCard === label;

    return (
      <div className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <button
          type="button"
          onClick={() => setActiveSummaryCard((current) => (current === label ? null : label))}
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <div className={`rounded-lg px-2 py-1 text-xs font-semibold ${isOpen ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600'}`}>
              {isOpen ? 'Hide' : 'View'}
            </div>
          </div>
        </button>
        ...
        {isOpen && (
          <div className="absolute left-0 top-full z-50 mt-2 w-80 rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-2xl">
            <div className="mb-2 flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setActiveSummaryCard(null)}
                className="rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-600 transition hover:bg-slate-100"
              >
                Close
              </button>
            </div>
