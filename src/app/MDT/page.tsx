...
export default function MDT(){
 ...
 const availableRadioIdentifiers=useMemo(()=>radioIdentifiersForVehicle(selectedVehicle),[selectedVehicle]);
  ...
  return <main className={`mdt-root shell ${night?"night":"day"}`} style={{filter:`brightness(${brightness/100})`}}>
    ...
    <footer className="carplayDock"><button className="active"><Map size={16}/>Call / Map</button><button onClick={()=>setMsgModal(true)}><MessageSquareText size={16}/>Messages</button><button><Hospital size={16}/>Hospitals</button><button onClick={()=>setDelayModal(true)}><Construction size={16}/>Delay</button><button onClick={()=>setStatusModal(true)} disabled={status==="Out of Service"}><SlidersHorizontal size={16}/>Status</button></footer>
